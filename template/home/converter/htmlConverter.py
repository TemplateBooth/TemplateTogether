				...
				if word.replace(' ', '')[-6:] == 'style=':
					...
					for value in values:
						...
						if len(current) == 2:
							...
							# extraaa
							if re.search('^--[a-zA-Z0-9_-]*', first):
								print("here")
								print("Fd")

								if re.search('#(?:[0-9A-Fa-f]{3}){1,2}|#(?:[0-9A-Fa-f]{4}){1,2}|rgba\(\s*(?:[01]?[0-9][0-9]?|[2][0-4][0-9]|25[0-5])\s*,\s*(?:[01]?[0-9][0-9]?|[2][0-4][0-9]|25[0-5])\s*,\s*(?:[01]?[0-9][0-9]?|[2][0-4][0-9]|25[0-5])\s*,\s*[01]?.?[0-9]?\s*\)|rgb\(\s*(?:[01]?[0-9][0-9]?|[2][0-4][0-9]|25[0-5])\s*,\s*(?:[01]?[0-9][0-9]?|[2][0-4][0-9]|25[0-5])\s*,\s*(?:[01]?[0-9][0-9]?|[2][0-4][0-9]|25[0-5])\s*\)', current):

									if re.search('^rgb*', current):
										newColor = self.rgb_tohex(current)
									elif current[0] == '#' and len(current) <= 5:
										newColor = self.halfhex_tofull(current)
									else:
										newColor = current

									if newColor in color_variables:
										# remove statement
										old_variables['var(' + first + ')'] = 'var(' + \
								...
								elif current.lower() in colorValues:
									if colorValues[current.lower()].lower() in color_variables:

										# remove statement
										old_variables['var(' + first + ')'] = 'var(' + \
											color_variables[colorValues[current.lower(
											)]] + ')'
									else:
										total_colors = total_colors + 1
										color_variables[colorValues[current.lower()].lower()] = "--color_" + \
											str(total_colors)
										old_variables['var(' + first + ')'] = 'var(' + \
											color_variables[colorValues[current.lower(
											)].lower()] + ')'
							...
							# extra end
								...
								# print(current)
								colors = re.findall('#(?:[0-9A-Fa-f]{3}){1,2}|#(?:[0-9A-Fa-f]{4}){1,2}|rgba\(\s*(?:[01]?[0-9][0-9]?|[2][0-4][0-9]|25[0-5])\s*,\s*(?:[01]?[0-9][0-9]?|[2][0-4][0-9]|25[0-5])\s*,\s*(?:[01]?[0-9][0-9]?|[2][0-4][0-9]|25[0-5])\s*,\s*[01]?.?[0-9]?\s*\)|rgb\(\s*(?:[01]?[0-9][0-9]?|[2][0-4][0-9]|25[0-5])\s*,\s*(?:[01]?[0-9][0-9]?|[2][0-4][0-9]|25[0-5])\s*,\s*(?:[01]?[0-9][0-9]?|[2][0-4][0-9]|25[0-5])\s*\)|var\(--[a-zA-Z0-9_-]*\)', current)
								# print(colors)
								texts = current.split(' ')
								for text in texts:
